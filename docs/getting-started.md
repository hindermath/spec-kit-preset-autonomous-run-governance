...
3. `autonomous-run-governance >=0.3.5` ist aktiv.
...
Erwartet werden Version `0.3.5`, Prioritaet `70` und genau ein Beitrag je
autonomem Befehl.
...
3. `autonomous-run-governance >=0.3.5` is active.
...
Expect version `0.3.5`, priority `70`, and one contribution for each autonomous
command.
