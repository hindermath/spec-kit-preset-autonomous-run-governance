...
3. `autonomous-run-governance >=0.3.3` ist aktiv.
...
Erwartet werden Version `0.3.3`, Prioritaet `70` und genau ein Beitrag je
autonomem Befehl.
...
3. `autonomous-run-governance >=0.3.3` is active.
...
Expect version `0.3.3`, priority `70`, and one contribution for each autonomous
command.
